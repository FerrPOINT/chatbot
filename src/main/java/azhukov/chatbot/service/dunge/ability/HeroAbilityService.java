...
public class HeroAbilityService {
    ...
    private synchronized String applySpecialAbility(HeroInfo targetHero, HeroClass targetHeroType, HeroInfo previousHero) {
        ...
        switch (targetHeroType) {
            ...
            case NECRO:
                if (previousHero != null) {
                    if (previousHero.isDead() && previousHero.getLevel() < targetHero.getLevel()) {
                    ...
                    return targetHeroType.getAbilityName() + ": невозможно воскресить такого жирнича: " + previousHero.getName();
                }
                return targetHeroType.getAbilityName() + ": предыдущий герой не найден.";
            case NOBLE:
                updateAbilityData(targetHero);
            ...
            case WEREWOLF:
                ...
                return targetHeroType.getAbilityName() + ": использована способность " + randomClass.getAbilityName() + ". " + applySpecialAbility(targetHero, randomClass, previousHero);
            case ROGUE:
                if (previousHero != null) {
